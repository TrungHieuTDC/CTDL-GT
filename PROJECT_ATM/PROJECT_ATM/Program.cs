...
internal class Program
{
    ...
    //test
    static string test(LinkedList<User> L)
    {
        int dem = 0;
        string KT = "";
        bool check = false;
        ...
        for (LinkedListNode<User> p = L.First; p != null; p = p.Next)
        {
             if (p.Value.Id == id && p.Value.Pin == pinn)
            {
                KT = p.Value.Id;
                goto end;
            }
            else if (p.Value.Id == id && p.Value.Pin != pinn)
            {
                p.Value.dem++;
                ...
                else
                {
                    ...
                    for (LinkedListNode<User> q = L.First; q != null; q = q.Next)
                    {
                        if (p.Value.Id == id)
                        {
                            p.Value.Pin = "7777";
